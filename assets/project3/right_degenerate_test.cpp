 ...
 * increasing order. This will generate a degenerate tree that leans to the
 * right! (also known as a way too compilcated list)
 * 
 */
int main () {
     ...
     * The Employees_Ascending vector is in data.h
     */

    auto& employees = Employees_Ascending;
    ...
    for (auto& expected : employees) {
        ...
        if (index >= employees.size()) {
            ...
            std::cout << "This value is outside the range of Employees_Ascending: [0, " << employees.size() -1 << "]";
            std::cout << std::endl;
            continue;
        }
